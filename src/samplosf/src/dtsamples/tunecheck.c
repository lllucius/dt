...
#define NUMBER_OF_VOICES	18	// Number of voices (times 2 for 8 bit voices)
...
inline int checkmax(S32 value,S32 checkval);
...
int TunedTable[NUMBER_OF_VOICES+1][(sizeof(SPDEFS)/sizeof(short))+1] = { 0 };	// hold all of the tuned params for Full Tune
...
int	main(int argc, char *argv[])
{
	...
	for (argp=1; argp<argc; argp++)
	{
		...
		case 'a':
			...
			sleep(3);	// Make sure user sees warning before it scrolls off screen
			break;
...
void DoAutotune()
{
	...
	{
		printf("TextToSpeechStartupEx() returned %u\n",mmStatus);
		exit(10);
	}
	...
	(*((LPTTS_HANDLE_T*)(ttsHandlePtr)))->hTuneStopEvent=OP_CreateEvent(TRUE,FALSE);
...
//		bSynthIsRunning = TRUE; // allow buffers..
		...
		if (mmStatus != MMSYSERR_NOERROR)
		{
			printf("TextToSpeechOpenInMemory() returned %u\n",mmStatus);
			exit(10);
		}
		...
		for (iJ=0;iJ<BUFS_PER_INSTANCE;iJ++)
		{
			...
			if (mmStatus != MMSYSERR_NOERROR)
			{
				printf("TextToSpeechAddBuffer() returned %u\n",mmStatus);
				exit(11);
			}
		}
		...
		OP_ResetEvent((*((LPTTS_HANDLE_T*)(ttsHandlePtr)))->hTuneStopEvent);
		...
		while (!bDone)
		{
			...
			OP_WaitForEvent((*((LPTTS_HANDLE_T*)(ttsHandlePtr)))->hTuneStopEvent,OP_INFINITE);
			//Sleep(50);
		...
		// check the results..
		{
			if(vtdData.bDidOverload)
			{
				fprintf(fpResultFile,"\r ** Overload detected at phoneme %d, previous phoneme %d\n:",
...
#endif
#if 0
			printf("*maximum value of downsampled voice :            %5d\n" ,(int)vtdData.purevmax);
			printf("*maximum value within filter of casade rnz:      %5d\n" ,(int)vtdData.rnzmax);
			printf("*maximum value within filter of casade rnp:      %5d\n" ,(int)vtdData.rnpmax);
			printf("*maximum value within filter of casade 1(F5):        %5d\n" ,(int)vtdData.c5max);
			printf("*maximum value within filter of casade 2(F4):        %5d\n" ,(int)vtdData.c4max);
			printf("*maximum value within filter of casade 3(F3):        %5d\n" ,(int)vtdData.c3max);
			printf("*maximum value within filter of casade 4(F2):        %5d\n" ,(int)vtdData.c2max);
			printf("*maximum value within filter of casade 5(F1):        %5d\n" ,(int)vtdData.c1max);
#endif
...
//			}
	...
	free(pcBufArray);
	//free(szInputData);
	OP_DestroyEvent((*((LPTTS_HANDLE_T*)(ttsHandlePtr)))->hTuneStopEvent);
...
VOID TTSCallbackRoutine(LONG lParam1, LONG lParam2,
		...
		DWORD dwIndexSampleNumber=0;
			...
			if (bVerbose)
			{
				fprintf(fpResultFile,"[%09u] [%02u] Buffer Turn:%lx\n",
					dwTime,dwInstanceParam,lParam2);
			
			
			// look for index marks..

				fprintf(fpResultFile,"        [%02u] %u index marks, %u bytes\n",
					dwInstanceParam,
			...
			for (iI=0;iI<(int)lpttsbBuffer->dwNumberOfIndexMarks;iI++)
			{
				...
				case 2:
					// end
					bGotDoneMark = TRUE;
				OP_SetEvent((*((LPTTS_HANDLE_T*)(ttsHandlePtr)))->hTuneStopEvent);
		...
		if (bVerbose)
		{
			if (dwTotalBuffers)
			{
				...
				{
					fprintf(fpResultFile,
						"[%09u][%02u] samp/sec: avg:%ld, curr:%ld\n",
						dwTime,
						dwInstanceParam,
			...
			else
			{
				dwFirstBufferTime = dwTime;
				fprintf(fpResultFile,
					"[%09u][%02u] time to first buffer=%u\n",
					dwTime,
					dwInstanceParam, 
					dwTime-dwStartTime);
				}
...
void StartText()
{
	...
	if ((ttsHandlePtr != NULL) 	)
	{
		...
		if (bVerbose)
		{
			fprintf(fpResultFile,"[%09u] TTSSpeak completed.\n", dwStartTime);
		}
	}
}
...
void CreateOffsetFile()
{
	...
	for(i=0;i<NUMBER_OF_VOICES+1;i++)
	{
		switch (i) {
		...
		case 9: speaker = "paul_tune";			break;
		case 10: speaker = "betty_tune";		break;
		case 11: speaker = "harry_tune";		break;
		case 12: speaker = "frank_tune";		break;
		case 13: speaker = "kit_tune";			break;
		case 14: speaker = "ursula_tune";		break;
		case 15: speaker = "rita_tune";			break;
		case 16: speaker = "wendy_tune";		break;
		case 17: speaker = "dennis_tune";		break;
		case 18: speaker = "val_tune";			break;
			
		}
		
		if(!create_empty_file && i != NUMBER_OF_VOICES) // Val has no defaults
		{
			DefaultParams = TextToSpeechGetPhVdefParams(ttsHandlePtr, i);
		}
		fprintf(fpOffsetFile, "\r\nconst short\t\t\t%s%s[] =\r\n{\r\n",lang_tag,speaker);
		
		if(!create_empty_file && i != NUMBER_OF_VOICES)
			printf("%s - ", speaker);
		for(j=0;j<(sizeof(SPDEFS)/sizeof(short));j++)
		{
			if(!create_empty_file &&  (i!=NUMBER_OF_VOICES) && (j>=GF && j<=LO)	)// get differences for just these values
			{
				fprintf(fpOffsetFile, "\t%d,\r\n", TunedTable[i][j] - DefaultParams[j]);
				printf("%d:%d:%d ", TunedTable[i][j],DefaultParams[j],TunedTable[i][j] - DefaultParams[j]);
			}
			else
		...
		if(i != NUMBER_OF_VOICES) // Val has no defaults
		{
	...
	printf("\n\nAll Done!!\n%s created in %s ", OFFSETFILE, offsetPath);
...
void DoFullAutoTune()
{
	...
	for(ii=0;ii<NUMBER_OF_VOICES+1;ii++)		// Initialize tables for full tune
	{
	...
	for(nextVoice=startVoice;nextVoice<NUMBER_OF_VOICES;nextVoice++)
	{
		...
		if(nextVoice < (NUMBER_OF_VOICES/2))
		{
			uiSampleRate = 8000;
			uiBytesPerSample = 1;
		}
		...
		printf("Voice %d/%d (%.0f%%):\n", nextVoice+1, NUMBER_OF_VOICES, ((100.0/NUMBER_OF_VOICES)*nextVoice));
		DoAutotune();
		printf("\r\t\t\t\t\t\t\t\r");
...
void ShowHelpMsg()
{
	...
	printf("-e               Create empty tuning file, prerequisite for autotuning.\n");
